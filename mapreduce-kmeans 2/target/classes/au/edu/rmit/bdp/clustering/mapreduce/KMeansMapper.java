...
/**
 * First generic specifies the type of input Key. Second generic specifies the
 * type of input Value. Third generic specifies the type of output Key. Last
 * generic specifies the type of output Value. In this case, the input key-value
 * pair has the same type with the output one.
 *
 * The difference is that the association between a centroid and a data-point
 * may change. This is because the centroids has been recomputed in previous
 * reduce().
 */
public class KMeansMapper extends Mapper<Centroid, DataPoint, Centroid, DataPoint> {
	...
	/**
	 *
	 * In this method, all centroids are loaded into memory as, in map(), we are
	 * going to compute the distance (similarity) of the data point with all
	 * centroids and associate the data point with its nearest centroid. Note that
	 * we load the centroid file on our own, which is not the same file as the one
	 * that hadoop loads in map().
	 *
	 *
	 * @param context Think of it as a shared data bundle between the main class,
	 *                mapper class and the reducer class. One can put something into
	 *                the bundle in KMeansClusteringJob.class and retrieve it from
	 *                there.
	 *
	 */
//	@SuppressWarnings("deprecation")
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		/**
		 * 
		 * After having the location of the file containing all centroids data, we read
		 * them using SequenceFile.Reader, which is another API provided by hadoop for
		 * reading binary file The data is modeled in Centroid.class and stored in
		 * global variable centers, which will be used in map()
		 *
		 */
...
//		try (SequenceFile.Reader reader = new SequenceFile.Reader(fs, centroids, conf)) {
	...
	/**
	 *
	 * After everything is ready, we calculate and re-group each data-point with its
	 * nearest centroid, and pass the pair to reducer.
	 *
	 * @param centroid  key
